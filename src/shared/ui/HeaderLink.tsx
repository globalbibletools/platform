  ...
  return (
    <NavLink
      ...
      className={(isActive) => `
            ...
            hover:underline focus:underline focus:outline-hidden
            ${
              isActive ?
...
export function HeaderDropdown({ button, items }: HeaderDropdownProps) {
  return (
    <Menu as="div" className="relative h-full hidden sm:block">
      <MenuButton
        className="h-full ps-2 font-bold focus:outline-hidden focus:underline text-blue-800 dark:text-green-400
        "
      >
        {({ open }) => (
          <>
...
export function HeaderMenuItem({
  ...
  return (
    <Link
      {...props}
      className={`
                h-8 px-4 py-1 whitespace-nowrap focus:underline hover:underline focus:outline-hidden text-blue-800 dark:text-green-400
                ${className}
            `}
      target={newTab ? "_blank" : props.target}
      rel={newTab ? "noopener" : props.rel}
