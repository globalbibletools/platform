...
import DashboardCard from "@/modules/dashboard/ui/DashboardCard";
...
function DashboardRoute() {
  ...
  const segregatedContributionData = contributionData.map((week) => ({
    ...week,
    currentUser: week.users.find((u) => u.userId === user.id)?.glosses ?? 0,
    otherUsers: week.users.reduce((total, u) => {
      if (u.userId === user.id) {
        return total;
      }

      return total + u.glosses;
    }, 0),
  }));
  const max = roundMax(
