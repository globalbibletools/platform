...
export default function ListboxInput(
    ...
    return <div ref={root} className={`${className} relative ${disabled ? 'opacity-25' : ''}`}>
        <Listbox
            value={value}
        ...
        >
            <Listbox.Button onBlur={onBlur} className="font-bold text-blue-800 dark:text-green-400 w-full outline-2 outline-green-300">
                {
                    ({ value, open }) => <>
                            {items.find(item => item.value === value)?.label ?? ''}
                            <Icon icon={open ? 'caret-up' : 'caret-down'} fixedWidth className="ms-1" />
                        </>
                }
            </Listbox.Button>
            <Listbox.Options
