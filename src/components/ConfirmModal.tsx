...
const ConfirmModal = forwardRef<ConfirmModalRef, ConfirmModalProps>(({ prompt, ...props }: ConfirmModalProps, ref) => {
    ...
    >
        ...
        <div className="mt-4 flex justify-end gap-2 items-center">
            <Button variant="secondary" onClick={(e: MouseEvent) => e.currentTarget.closest('dialog')?.close('no')}>{t("no")}</Button>
            <Button onClick={(e: MouseEvent) => e.currentTarget.closest('dialog')?.close('yes')}>{t("yes")}</Button>
        </div>
    </dialog>
})
