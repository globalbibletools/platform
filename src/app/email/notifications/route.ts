    ...
    case "Notification": {
      ...
      switch (message.notificationType) {
        case "Bounce": {
          ...
          const emails = message.bounce.bouncedRecipients.map((r) =>
            r.emailAddress.toLowerCase(),
          );
          console.log(`Email bounced: ${emails.join(", ")}`);
          await query(
            `UPDATE users SET email_status = 'BOUNCED' WHERE email = ANY($1::text[])`,
            [emails],
          );
        ...
        case "Complaint": {
          const emails = message.complaint.complainedRecipients.map((r) =>
            r.emailAddress.toLowerCase(),
          );
          console.log(`Email complaint: ${emails.join(", ")}`);
          await query(
            `UPDATE "User" SET email_status = 'COMPLAINED' WHERE email = ANY($1::text[])`,
            [emails],
          );
