...
const predictGlossesSchema = z.object({
    code: z.string(),
    verseId: z.string()
})
...
export async function predictGlosses(_prev: FormState, formData: FormData): Promise<FormState> {
    const session = await verifySession()
    if (!session?.user) {
        notFound()
    }

    const request = predictGlossesSchema.safeParse(parseForm(formData));
    if (!request.success) {
        return { state: 'error' }
    }
