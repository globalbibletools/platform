...
import { getReadAudioTimings } from "../serverFns/getReadAudioTimings";
...
export default function AudioDialog({
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      return getReadAudioTimings({ data: { speaker, chapterId } });
    },
  });
...
function ScrubBar({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="bg-blue-800 dark:bg-green-400 absolute h-4 w-4 -top-1 -ml-2 rounded-full pointer-cursor group-focus:outline-2 outline-green-300"
        style={{
