...
test("when returnIfActive is false, throws error if user is already active", async () => {
...
test("when returnIfActive is true, returns user id for active user", async () => {
  const email = "TEST@example.com";
  const user = new User({
    id: ulid(),
    email: new UserEmail({
  ...
  const result = await inviteUser({ email, returnIfActive: true });
  expect(result).toEqual({ userId: user.id });
});

test.each([false, true])(
  "when returnIfActive is %s, recreates and resends invite for pending user",
  async (returnIfActive) => {
    const email = "TEST@example.com";
    const props = {
      id: ulid(),
      email: new UserEmail({
    ...
    const result = await inviteUser({ email, returnIfActive });
    expect(result).toEqual({ userId: user.id });
