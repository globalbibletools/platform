...
export async function restoreSnapshotJob(job: RestoreSnapshotJob) {
  ...
  if (job.type !== SNAPSHOT_JOB_TYPES.RESTORE_SNAPSHOT) {
    jobLogger.error(
      `received job type ${job.type}, expected ${SNAPSHOT_JOB_TYPES.RESTORE_SNAPSHOT}`,
    );
    throw new Error(
      `Expected job type ${SNAPSHOT_JOB_TYPES.RESTORE_SNAPSHOT}, but received ${job.type}`,
    );
  }
