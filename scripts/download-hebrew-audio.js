...
const BASE_URL = 'https://f005.backblazeb2.com/file/BARTMedia/SoundFiles'
...
const BUCKET = 'gbt-audio'

const s3Client = new S3Client({})

async function downloadFile(version, bookId, chapter) {
    const bookName = FULL_BOOKS[bookId]
    const bookShortName = SHORT_BOOKS[bookId]

    const url = `${BASE_URL}/${version}/${bookName}/${bookShortName}${chapter.toString().padStart(3, '0')}.mp3`
    const response = await fetch(url)
    const blob = await response.blob()
    return blob
}

async function uploadFile(blob, version, bookId, chapter) {
    const key = `${version}/${SHORT_BOOKS[bookId]}/${chapter.toString().padStart(3, '0')}.mp3`
...
async function transferFile(version, bookId, chapter) {
    console.log(`Transferring ${version} ${SHORT_BOOKS[bookId]} ch ${chapter}`)
    const blob = await downloadFile(version, bookId, chapter)
    await uploadFile(blob, version, bookId, chapter)
}
...
async function transferOT(version) {
    for (let b = 0; b < SHORT_BOOKS.length; b++) {
        await transferBook(version, b)
    }
}
