...
- **Background jobs** live in `src/shared/jobs/` and module `jobs/` directories; the lambda worker entrypoint is `src/shared/jobs/bin/lambdaWorker.ts` and the dev worker entrypoint is `src/shared/jobs/bin/localWorker.ts`.
