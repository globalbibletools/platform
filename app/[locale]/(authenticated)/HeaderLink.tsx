...
export function HeaderLink({ className = '', newTab = false, children, ...props }: HeaderLinkProps) {
    return <NavLink
        {...props}
        className={(isActive) => `
...
export function HeaderDropdown({ button, items }: HeaderDropdownProps) {
    return <Menu as="div" className="relative h-full hidden sm:block">
        <Menu.Button className="h-full ps-2 font-bold focus:outline-none focus:underline text-blue-800 dark:text-green-400
        ">
            {({ open }) => <>
                {button} 
...
export function HeaderDropdownItem({ children, className, newTab = false, ...props }: HeaderLinkProps) {
    return <Menu.Item>
        <Link
            {...props}
            className={`
...
export function HeaderMenu({ children }: { children: ReactNode }) {
    ...
    return <HeaderMenuContext.Provider value={{isOpen, setOpenState}}>
        {children}
...
export function HeaderMenuItems({ children, className = '' }: { children: ReactNode, className?: string }) {
    const context = useContext(HeaderMenuContext)
...
export function HeaderMenuItem({ children, className, newTab = false, ...props }: HeaderLinkProps) {
        const context = useContext(HeaderMenuContext)
