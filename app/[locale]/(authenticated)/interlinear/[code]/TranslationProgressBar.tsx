...
export default function TranslationProgressBar() {
    const { locale, verseId, code } = useParams<{ locale: string, verseId: string, code: string }>()
    ...
    const [{ textOffset, fitsInside }, setTextStyle] = useState({ textOffset: 0, fitsInside: false });
    ...
    return <div className="relative h-2 w-full">
        <div className={`absolute group w-full h-2 hover:h-6 top-0 start-0 z-[1] ${isLoading ? 'bg-gray-100' : 'bg-brown-100'}`}>
            <div
            ...
            <div className="absolute hidden group-hover:block text-xs select-none top-0 start-0 w-full h-full px-6 md:px-8">
                <div className={`h-6 leading-6 ${fitsInside ? 'text-white' : ''}`} style={{ translate: `${textOffset}px 0px` }}>
                    {description}
                </div>
            </div>
        </div>
