        ...
        <div className="relative min-h-screen flex flex-col overflow-x-hidden">
            ...
            >
                <Link href="/" className="flex items-center me-8">
                    <img src="/bet-scroll.png" className="w-10 h-10" alt="" aria-hidden="true" />
                    <h1 className="font-bold ms-2 text-lg">
                        {t('app_name')}
                    </h1>
                </Link>
                        ...
                        <HeaderDropdown
                            button={
                                <>
                                    ...
                                    <span className="sr-only md:not-sr-only md:inline">{session?.user.name ?? ''}</span>
                                </>
                            }
                            items={
                                <>
                                    ...
                                    <HeaderDropdownItem href={`/${params.locale}/logout`} prefetch={false}>
                    ...
                    <HeaderMenuItems className="sm:hidden">
                        {session ?
                            <HeaderMenuItem href={`/${params.locale}/profile`}>
                                <Icon icon="user" className="me-2" fixedWidth />
                                <span className="font-bold">{session?.user.name ?? t('links.profile')}</span>
                            </HeaderMenuItem>
                        ...
                        {session && <>
                            ...
                            <HeaderMenuItem href={`/${params.locale}/logout`} prefetch={false}>
                                <Icon icon="right-from-bracket" className="me-2" fixedWidth />
                                <span className="font-bold">{t('links.log_out')}</span>
                            </HeaderMenuItem>
                        </>}
